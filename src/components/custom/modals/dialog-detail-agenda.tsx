...
const DialogDetailAgenda = (props: DialogDetailAgendaProps) => {
  ...
  const [selectedSTatus, setSelectedStatus] = useState<any>(evt.resource?.status)

  useEffect(() => {
    const updateStatus = async (status: string) => {
      const session = await getSession()
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/agenda/${evt.resource?.id}/status`, {
        method: 'POST',
        headers: {
      ...
      <DialogContent>
        ...
        <table className="table w-full text-left mb-5">
          <tr>
            ...
            <th><span className="px-2">:</span></th>
        ...
        {!evt.resource?.hasSurat && (
          ...
          <div className="flex gap-4">
            ...
            <Button
              ...
              variant="destructive"
              onClick={() => {
                const c = window.confirm('Apakah anda yakin ingin menghapus agenda ini?')
                if (c) {
