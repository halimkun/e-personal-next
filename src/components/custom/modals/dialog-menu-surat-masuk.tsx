...
const DialogMenuSuratMasuk = (props: DialogMenuSuratMasukProps) => {
  const { isOpenMenu, setIsOpenMenu, setIsOpenFormAdd, selectedItem, mutate } =
    props;
  ...
  async function handleDelete() {
    ...
    if (confirm) {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/surat/masuk/delete/${selectedItem.no}`,
        {
          method: 'DELETE',
          headers: {
    ...
    <Dialog open={isOpenMenu} onOpenChange={() => setIsOpenMenu(false)}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Anda dapat melakukan manipulasi data terhadap surat masuk ini dengan
            beberapa menu yang tersedia di bawah ini.
          </DialogDescription>
        </DialogHeader>
        ...
        <table className='table w-full text-left'>
          <tr className='align-top'>
            ...
            <td>
              {new Date(selectedItem.no_simrs).toLocaleDateString('id-ID', {
                year: 'numeric',
                month: '2-digit',
                day: '2-digit',
              })}
        ...
        <div className='mt-6 flex flex-row items-center justify-end gap-2'>
          <Link
            href={`/surat/masuk/${selectedItem.no}/edit`}
            className={cn(
              buttonVariants({
                variant: 'default',
                size: 'sm',
