  ...
  return (
    <Dialog open={props.open} onOpenChange={props.onOpenChange}>
      <DialogContent className='max-w-2xl'>
        <DialogHeader>
          ...
          <DialogDescription>
            <div className='flex flex-wrap items-start justify-start gap-1'>
              ...
              >
                <IconUserShield size={16} strokeWidth={2} />{' '}
                {item.perihal?.pj_detail?.nama ?? ''}
              </Badge>
              <Badge
                className='flex items-center justify-center gap-1'
                title='Dari'
              ...
              >
                ...
                {getDate(item.perihal?.tgl_terbit)}
              </Badge>
            </div>

            <div className='mt-4 flex items-start justify-start gap-1'>
              {/* loop additionalItem.mengetahui */}
              {mengetahui &&
                mengetahui.length > 0 &&
                mengetahui.map((item: any, index: number) => (
                  <Badge
                    variant='success'
                    key={index}
                    className='flex items-center justify-center gap-1'
        ...
        <div
          dangerouslySetInnerHTML={{ __html: decodedHTML(item.content) || '' }}
        />
        <Separator className='mt-2' />
        {/* looop penerima */}
        <div className='flex flex-wrap items-start justify-start gap-1'>
          {penerima &&
            penerima.length > 0 &&
            penerima.map((item: any, index: number) => (
              <Badge
                variant='secondary'
                key={index}
                className='flex items-center justify-center gap-1'
