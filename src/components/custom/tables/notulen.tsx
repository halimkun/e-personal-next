...
const TableNotulen = ({
  ...
  const onDelete = async (nomor: string) => {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/berkas/notulen/delete`,
      {
        method: 'POST',
        headers: {
  ...
  const columns = [
    ...
    {
      // edit and delete
      name: <div className='flex w-full justify-end'>#</div>,
      selector: 'action',
      enableHiding: false,
      data: (row: any) => (
        <DropdownMenu>
            ...
            <DropdownMenuGroup>
              <DropdownMenuGroup>
                <DropdownMenuItem
                  ...
                  onClick={() => {
                    route.push(
                      `/kegiatan/${row.no_surat.split('/').join('--')}/kehadiran`
                    );
                  }}
                >
