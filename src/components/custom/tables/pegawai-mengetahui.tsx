...
const TablePegawaiMengetahui = (props: TablePegawaiMengetahuiProps) => {
  ...
  const { data, error, mutate, isLoading, isValidating } = useSWR(
    `${process.env.NEXT_PUBLIC_API_URL}/pegawai/get/mengetahui`,
    fetcher,
    {
      ...
      data: (row: any) => (
        <div className='px-2'>
          <Checkbox
            ...
            disabled={
              selectedMengetahui.length >= maxMengetahui &&
              !selectedMengetahui.includes(row.nik)
            }
            onCheckedChange={() => {
              if (selectedMengetahui.includes(row.nik)) {
                setSelectedMengetahui(
                  selectedMengetahui.filter((item) => item !== row.nik)
                );
              } else {
  ...
  return (
    <Card className='top-16 w-full lg:sticky'>
      <CardHeader>
        ...
        <CardDescription>
          Anda dapat memilih maksimal 2 orang yang mengetahui memo ini dengan
          tabel dibawah ini.
        </CardDescription>
      </CardHeader>
      <CardContent>
