...
const FormUploadBerkasPegawai = ({ nik, setOpen }: any) => {
  ...
  useEffect(() => {
    const fetchKategori = async () => {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/pegawai/berkas/kategori`, {
        headers: {
  ...
  const fetchBerkas = async (kategori: string) => {
    setBerkas([])
    const session = await getSession()
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/pegawai/berkas/nama-berkas?kategori=${kategori}`, {
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${session?.rsiap?.access_token}`
      }
  ...
  const handleSubmit = async (e: any) => {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/pegawai/upload/berkas`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${session?.rsiap?.access_token}`
      },
