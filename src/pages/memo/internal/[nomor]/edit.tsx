...
const EditMemoInternal: NextPageWithLayout = (props: any) => {
  ...
  return data.success ? (
    <FormAddMemoInternal data={data.data} />
  ) : (
    <div className='w-fit rounded-lg bg-warning/90 p-5 dark:bg-warning/60'>
      <h1 className='text-lg font-bold'>Data tidak ditemukan</h1>
      <p className='text-base'>
        Data memo internal dengan nomor surat{' '}
        <span className='font-bold'>{nomorSurat}</span> tidak ditemukan
      </p>
    </div>
  );
};
...
export async function getServerSideProps(ctx: any) {
  ...
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/berkas/memo/internal/${nomor}/show`,
    {
      method: 'GET',
      headers: {
