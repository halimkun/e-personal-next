...
const UploadBerkasKaryawan = dynamic(
  () => import('@/components/custom/modals/upload-berkas-karyawan'),
  { ssr: false }
);
...
const BerkasKaryawan = () => {
  ...
  const { data: berkas, error } = useSWR(
    `${process.env.NEXT_PUBLIC_API_URL}/pegawai/get/berkas`,
    fetcher
  );
            ...
            <div className='flex items-center gap-4'>
              ...
              <div className='flex w-full items-center justify-between'>
                <div className='space-y-1'>
                  ...
                  <CardDescription>
                    Berkas Karyawan <strong>RSIA Aisyiyah Pekajangan</strong>
                  </CardDescription>
                </div>
          ...
          <CardContent className='space-y-3'>
            ...
            <div className='grid grid-cols-1 gap-3 md:grid-cols-2 2xl:grid-cols-3'>
              {data.berkas.map((item: any, index: number) => {
                return (
                  <div
                    className='flex flex-col justify-between rounded-lg border bg-background p-4 transition-all duration-200 ease-in-out hover:scale-[.97]'
                    key={index}
                  >
                    <div>
                      <div className='flex flex-col gap-1'>
                        <p className='text-lg font-bold'>
                          Berkas {item.master_berkas_pegawai.nama_berkas}
                        </p>
                    ...
                    <div className='flex items-center justify-end gap-2'>
                      <Button
                        ...
                        onClick={() =>
                          handleDelete(
                            item.master_berkas_pegawai.kode,
                            item.berkas
                          )
                        }
                      >
        ...
        <Card>
          ...
          <CardContent>
            <div className='flex flex-col gap-2'>
              <p className='text-sm'>
                Berkas karyawan dengan nik{' '}
                <Badge variant='outline'>{router.query.nik}</Badge> tidak
                ditemukan.
              </p>
            </div>
          </CardContent>
