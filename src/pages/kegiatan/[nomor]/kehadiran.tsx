...
const PenerimaUndanganTable = dynamic(
  () => import('@/components/custom/tables/penerima-undangan'),
  { ssr: false }
);
...
const KehadiranKegiatanPage = ({
  ...
  const {
    ...
    isLoading,
    isValidating,
  } = useSWR(`${process.env.NEXT_PUBLIC_API_URL}/undangan/penerima`, fetcher, {
  ...
  const onSubmit = async (e: any) => {
    ...
    toast.promise(
      fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/undangan/kegiatan/tambah/presensi`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${session?.rsiap?.access_token}`,
  ...
  return (
    <div className='flex flex-col gap-3'>
        ...
        <CardHeader className='p-3'>
          <div className='flex items-center gap-4'>
            ...
            <div className='flex flex-col gap-0.5'>
              ...
              <CardDescription>
                Data kehadiran karyawan dalam rapat |{' '}
      ...
      <form method='post' onSubmit={onSubmit}>
        {' '}
        {/* <<==== TODO: do submit instruction */}
        <div className='flex flex-col-reverse items-start gap-3 lg:flex-row'>
          <div className='flex w-full flex-col gap-3'>
            <Card className='w-full'>
              <CardHeader>
                <CardTitle>Data Penerima Undangan Kegiatan</CardTitle>
                <CardDescription>
                  Berikut ini adalah data penerima undangan kegiatan
                </CardDescription>
              </CardHeader>
              <CardContent>
            ...
            {isMoreKaryawan && (
              <Card className='w-full transition-all duration-300 fade-in fade-out'>
                <CardHeader>
                  <CardTitle>Data Karyawan</CardTitle>
                  <CardDescription>
                    Anda dapat menambahkan karyawan selain penerima undangan
                    untuk kehadiran,{' '}
                    <span className='text-warning'>
                      semua data yang terpilih pada tabel dibawah ini akan
                      dianggap hadir
                    </span>
                  </CardDescription>
                </CardHeader>
            ...
            <Card>
              <CardHeader className='p-3'>
                <div className='flex items-center justify-between'>
                  <CardTitle className='flex gap-5 text-primary'>
                    Kehadiran Karyawan
                  </CardTitle>
          ...
          <Card className='top-[67px] w-full bg-secondary lg:sticky lg:w-[55%]'>
            <CardHeader>
              <CardTitle>Informasi Kegiatan</CardTitle>
              <CardDescription>
                Berikut ini adalah informasi kegiatannya
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className='flex flex-col gap-1.5'>
                <div className='space-y-1'>
                  <p className='font-bold text-primary'>No. Surat</p>
                  {nomor}
                </div>
                <div className='space-y-1'>
                  <p className='font-bold text-primary'>Perihal</p>
                  {data.perihal}
                </div>
                <div className='space-y-1'>
                  <p className='font-bold text-primary'>Tempat</p>
                  {data.tempat}
                </div>
                <div className='space-y-1'>
                  <p className='font-bold text-primary'>Penanggung Jawab</p>
                  {data.penanggung_jawab.nama}
                </div>
                <div className='space-y-1'>
                  <p className='font-bold text-primary'>Tanggal & Waktu</p>
                  {getFullDateWithDayName(data.tanggal)} |{' '}
                  {getTime(data.tanggal)} WIB
                </div>
                <div className='space-y-1'>
                  <p className='font-bold text-primary'>Jumlah Peserta</p>
                  {data.penerima_count} orang ( {karyawanHadir.length} hadir )
                </div>
                <div className='space-y-1'>
                  <p className='font-bold text-primary'>Notulen</p>
                  <Badge variant={data.notulen ? 'success' : 'danger'}>
                    {data.notulen ? (
                      <>
                        <IconCircleCheck size={18} className='mr-1' /> sudah
                        dibuat
                      </>
                    ) : (
                      <>
                  ...
                  {data.notulen && (
                    <div className='py-4'>
                      <table className='w-full'>
                        <tr className='border-foreground/12 border-b'>
                        ...
                        <tr className='border-foreground/12 border-b'>
                          ...
                          <td>
                            {getFullDateWithDayName(data.notulen.created_at)}
                          </td>
                        </tr>
                      </table>
                    </div>
...
export async function getServerSideProps(context: any) {
  ...
  const kegiatan = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/undangan/detail`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
  ...
  const penerima = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/undangan/penerima`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
