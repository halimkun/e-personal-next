...
const TablePPI = dynamic(() => import('@/components/custom/tables/ppi'), {
  ssr: false,
});
...
const KomiteKeperawatanPage: NextPageWithLayout = () => {
  ...
  const { data, error, mutate, isLoading, isValidating } = useSWR(
    `${process.env.NEXT_PUBLIC_API_URL}/berkas/komite/keperawatan`,
    fetcher,
    {
  ...
  const onDelete = async (e: any) => {
    const con = confirm(
      'apakah anda yakin akan menghapus data dengan perihal -- ' +
        e.perihal +
        '?'
    );
    if (!con) return;
    ...
    toast.promise(
      fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/berkas/komite/keperawatan/delete`,
        {
          method: 'POST',
          headers: {
  ...
  return (
    <div>
      <div className='flex w-full items-center justify-between'>
        <div className='border-l-4 border-primary bg-gray-100 px-2 dark:bg-gray-900'>
          <h4 className='text-xl font-semibold text-primary'>
            Berkas Komite Keperawatan
          </h4>
        </div>
        <Link
          href='/berkas/komite/keperawatan/new'
...
KomiteKeperawatanPage.getLayout = function getLayout(page: ReactElement) {
  return <AppLayout>{page}</AppLayout>;
};

export default KomiteKeperawatanPage;
