...
const CreatePPIPage: NextPageWithLayout = () => {
  ...
  const onSubmit = async (e: any) => {
    ...
    // post to /berkas/ppi/store
    toast.promise(
      fetch(`${process.env.NEXT_PUBLIC_API_URL}/berkas/ppi/store`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="flex flex-col-reverse lg:flex-row items-start gap-6">
      <Card className="w-full">
        <CardHeader>
          <CardTitle className="text-primary">Form PPI</CardTitle>
          <CardDescription>Form untuk menambahkan data berkas PPI</CardDescription>
        </CardHeader>
        <CardContent>
          <FormAddPPI
      ...
      <Card className="w-[50%]">
        <CardHeader>
          ...
          <CardDescription>Informasi tambahan untuk menambahkan data berkas PPI</CardDescription>
        </CardHeader>
        <CardContent>
        </CardContent>
      </Card>
...
CreatePPIPage.getLayout = (page: ReactElement) => {
  return (
    <AppLayout>{page}</AppLayout>
  )
}

export default CreatePPIPage