...
const PMKPPage: NextPageWithLayout = () => {
  ...
  const { data, error, mutate, isLoading, isValidating } = useSWR(
    `${process.env.NEXT_PUBLIC_API_URL}/berkas/pmkp`,
    fetcher,
    {
  ...
  const onDelete = async (e: any) => {
    ...
    toast.promise(
      fetch(`${process.env.NEXT_PUBLIC_API_URL}/berkas/pmkp/delete`, {
        method: 'POST',
        headers: {
    ...
    <div>
      <div className='flex w-full items-center justify-between'>
        <div className='border-l-4 border-primary bg-gray-100 px-2 dark:bg-gray-900'>
          <h4 className='text-xl font-semibold text-primary'>Berkas PMKP</h4>
        </div>
        <Link
          href='/berkas/pmkp/new'
...
PMKPPage.getLayout = function getLayout(page: ReactElement) {
  return <AppLayout>{page}</AppLayout>;
};

export default PMKPPage;
