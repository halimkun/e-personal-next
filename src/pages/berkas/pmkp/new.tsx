...
const CreatePMKPPage: NextPageWithLayout = () => {
  ...
  const onSubmit = async (e: any) => {
    ...
    // post to /berkas/pmkp/store
    toast.promise(
      fetch(`${process.env.NEXT_PUBLIC_API_URL}/berkas/pmkp/store`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="flex flex-col-reverse lg:flex-row items-start gap-6">
      <Card className="w-full">
        <CardHeader>
          <CardTitle className="text-primary">Form PMKP</CardTitle>
          <CardDescription>Form untuk menambahkan data berkas PMKP</CardDescription>
        </CardHeader>
        <CardContent>
          <FormAddPPI
      ...
      <Card className="w-[50%]">
        <CardHeader>
          ...
          <CardDescription>Informasi tambahan untuk menambahkan data berkas PMKP</CardDescription>
        </CardHeader>
        <CardContent>
        </CardContent>
      </Card>
...
CreatePMKPPage.getLayout = (page: ReactElement) => {
  return (
    <AppLayout>{page}</AppLayout>
  )
}

export default CreatePMKPPage