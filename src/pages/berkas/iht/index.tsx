...
const BerkasIHTPage: NextPageWithLayout = () => {
  ...
  const { data, error, mutate, isLoading, isValidating } = useSWR(
    `${process.env.NEXT_PUBLIC_API_URL}/berkas/sertifikat`,
    fetcher,
    {
  ...
  const onDelete = async (e: any) => {
    ...
    toast.promise(
      fetch(`${process.env.NEXT_PUBLIC_API_URL}/berkas/sertifikat/delete`, {
        method: 'POST',
        headers: {
      ...
      <div className='flex w-full items-center justify-between'>
        <div className='border-l-4 border-primary bg-gray-100 px-2 dark:bg-gray-900'>
          <h4 className='text-xl font-semibold text-primary'>
            Berkas / Dokumen In House Training (IHT)
          </h4>
        </div>
        <Link
          href='/berkas/iht/new'
...
BerkasIHTPage.getLayout = function getLayout(page: ReactElement) {
  return <AppLayout>{page}</AppLayout>;
};

export default BerkasIHTPage;
