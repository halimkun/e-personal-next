...
const SpoDetailPage = () => {
    ...
    const getSpo = async () => {
      try {
        ...
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/berkas/spo/show?nomor=${nomor}`,
          {
            method: 'GET',
            headers: {
  ...
  async function onSubmit(e: any) {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/berkas/spo/detail/store`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${session?.rsiap?.access_token}`,
            ...
            <form onSubmit={onSubmit}>
              <div className='space-y-3'>
                ...
                <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
                  {['pengertian', 'tujuan', 'kebijakan', 'prosedur'].map(
                    (field) => (
