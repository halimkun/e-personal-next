    ...
    },
    {
      ...
      data: (row: any) => (
        <div className="flex gap-1">
          <Button variant={row.is_verified ? 'secondary' : 'outline'} size="icon" className={cn(
            'h-6 w-6',
            row.is_verified ? 'bg-green-500 dark:bg-green-600 text-white dark:text-gray-100' : 'text-gray-600 dark:text-gray-400'
          )}
            onClick={() => {
              setSpo(row)
              setOnConfirm(true)
      ...
      <Dialog open={onConfirm} onOpenChange={setOnConfirm}>
        <DialogContent>
          ...
          <div className="flex gap-3 w-full justify-end">
            <Button size={'sm'} variant="secondary" onClick={() => setOnConfirm(false)}>Batal</Button>
